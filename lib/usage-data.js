...
const PRICED_MODEL_ORDER = Object.freeze([
  "gpt-5.6-sol",
  "gpt-5.6-terra",
  "gpt-5.6-luna",
  "gpt-5.5-pro",
...
function canonicalizePricedModel(model) {
  ...
  const explicitAlias = MODEL_ALIASES[normalizedModel];
  if (explicitAlias) {
    return explicitAlias;
  }

  for (const pricedModel of PRICED_MODEL_ORDER) {
    if (isSnapshotAlias(normalizedModel, pricedModel)) {
      return pricedModel;
    }
  }
...
function buildHeatmapScale(activeTokenDays) {
  const maxTotalTokens = activeTokenDays.length ? Math.max(...activeTokenDays) : 0;
  const percentileCap = percentile(activeTokenDays, 0.9);
  const baselineTotalTokens = maxTotalTokens > 0
    ? Math.max(1, Math.min(maxTotalTokens, percentileCap || maxTotalTokens))
    : 0;
  const thresholds = [
    baselineTotalTokens * 0.08,
    baselineTotalTokens * 0.2,
    baselineTotalTokens * 0.45,
    baselineTotalTokens
  ];
...
function buildHeatmapDays(dayMap, range) {
  ...
  for (let cursor = new Date(range.startDate); cursor <= range.endDate; cursor = addDays(cursor, 1)) {
    ...
    if (value > 0) {
      activeTokenDays.push(value);
    }
  }

  const { maxTotalTokens, baselineTotalTokens, thresholds } = buildHeatmapScale(activeTokenDays);
  ...
  for (let cursor = new Date(alignedStart); cursor <= alignedEnd; cursor = addDays(cursor, 1)) {
    ...
    if (value > 0) {
      if (value <= thresholds[0] || baselineTotalTokens === 0) {
        level = 1;
  ...
  return {
    ...
    scale: {
      ...
      scale_method: "90th_percentile_cap",
      thresholds
    }
  };
}
...
function projectKeyForSession(session) {
  ...
  return `workspace:${session.workspace_key || "unknown"}`;
}
...
function getProjectAggregate(projectMap, session) {
  const key = projectKeyForSession(session);
  if (!projectMap.has(key)) {
    projectMap.set(key, createProjectAggregate(session));
  }
...
function serializeProjectUsage({
  ...
  return [...projectMap.values()]
    .map((entry) => {
      const previousEntry = previousProjectMap.get(entry.project_key) || emptyTotals();
      return {
...
function filterContributions(index, options) {
  ...
  const previousProjectMap = new Map();
  ...
  for (const session of index.sessions) {
    ...
    for (const event of session.events) {
      ...
      if (
        ...
        const previousProjectTotals = getProjectAggregate(previousProjectMap, session);
        addTotals(previousProjectTotals, pricedEvent);
        previousProjectTotals.active_days.add(event.date);
        previousProjectTotals.sessions.add(session.session_id);
      }
      ...
      const projectTotals = getProjectAggregate(projectMap, session);
...
export async function loadUsageIndex({
  ...
  for (const session of sessions) {
    sessionNameById.set(session.session_id, session.thread_name);
  }

  for (const session of sessions) {
    if (session.parent_session_id) {
      session.parent_thread_name =
        threadNames.get(session.parent_session_id) ||
        sessionNameById.get(session.parent_session_id) ||
        session.parent_thread_name ||
        null;
    }
    for (const event of session.events) {
